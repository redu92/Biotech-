...
if st.session_state.paso == 7:
    ...
    default_prompt = (
        ...
        f"Costo objetivo de la formulación: {st.session_state.costo_objetivo_kg:.2f} soles/kg "
        f"(este es el costo MÁXIMO que debes priorizar cumplir).\n\n"
        ...
        f"{st.session_state.costo_objetivo_kg:.2f} soles/kg.\n"
        f"Si con los ingredientes seleccionados el costo queda por ENCIMA del objetivo, ajusta la fórmula "
        f"reduciendo el porcentaje de los ingredientes más costosos y aumentando la participación de los más económicos, "
        f"siempre que sea técnicamente viable.\n\n"
        f"Además, considera como alternativas económicas para ajustar costos:\n"
        f"- Polidextrosa a S/ 5.78 por kg\n"
        f"- Almidón de maíz a S/ 3.50 por kg\n"
        f"Estas opciones pueden utilizarse como portadores o bases de relleno para ayudar a cumplir el costo objetivo sin afectar significativamente la funcionalidad.\n\n"
        f"Si aun así no es posible alcanzar el costo objetivo, puedes complementar la formulación agregando otros insumos "
        f"compatibles y de bajo costo (como bases neutras, rellenos o portadores), y disminuyendo en cantidad "
        f"los ingredientes más caros para aproximar el costo final lo máximo posible al objetivo.\n\n"
        ...
        f"describiendo qué ajustes hiciste (ingredientes disminuidos, aumentados o añadidos) para acercarte al presupuesto."
    )
